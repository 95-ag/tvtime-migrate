...
export function buildEpisodes(refractWatched, rescueEpisodes) {
  ...
  for (const r of refractWatched) {
    ...
    master.set(k, {
      showTvdb: r.seriesTvdb, showImdb: r.seriesImdb, showTitle: r.title,
      season: r.season, episode: r.episode, watchedAt: r.watchedAt,
      epTvdb: r.epTvdb, special: r.special, source: 'refract', dateConfidence: 'canonical',
    });
  }
  for (const s of rescueEpisodes) {
    ...
    master.set(k, {
      showTvdb: s.showTvdb, showImdb: null, showTitle: s.title,
      season: s.season, episode: s.episode, watchedAt: s.watchedAt,
      epTvdb: s.epTvdb, special: s.special, source: 'rescue', dateConfidence: 'insert-time',
    });
  }
