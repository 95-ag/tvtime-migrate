...
// core never fabricates replay dates (data-integrity rule). Unbridged rows are returned as `dropped`
// and land on the failure manifest at import time.
const norm = (s) =>
  String(s || '')
    .toLowerCase()
    .replace(/\s+/g, ' ')
...
export function buildRewatch(gdprRewatch, nameToTvdb) {
  ...
  for (const r of gdprRewatch) {
    ...
    bridged.push({ showTvdb: tvdb, season: r.season, episode: r.episode, plays: r.cpt });
  }
  bridged.sort(
    (a, b) =>
