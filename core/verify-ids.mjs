...
export function checkIdIntegrity(refractSeries, rescueShows) {
  ...
  const mismatches = [];
  let matched = 0, unjoinable = 0;
  for (const r of rescueShows) {
    if (!r.tvdb) continue;
    const rt = refractByName.get(norm(r.title));
    if (rt === undefined) { unjoinable++; continue; }
    if (rt === r.tvdb) matched++;
    else mismatches.push({ title: r.title, refract: rt, rescue: r.tvdb });
  }
  if (mismatches.length) {
    throw new Error(`id-integrity: ${mismatches.length} tvdb mismatch(es) across lineages: ${JSON.stringify(mismatches.slice(0, 5))}`);
  }
