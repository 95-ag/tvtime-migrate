...
export function buildMovies(movies) {
  return movies
    .map((m) => ({ tvdb: m.tvdb, imdb: m.imdb, uuid: m.uuid, title: m.title,
                   year: m.year, watchedAt: m.watchedAt, watched: m.watched }))
    .sort(byTvdb);
}
...
export function buildShows(refractSeries, rescueShows) {
  ...
  const shows = refractSeries.map((s) => ({
    tvdb: s.tvdb, imdb: s.imdb, title: s.title, status: s.status,
    simklBucket: statusToSimkl(s.status), traktTreatment: statusToTrakt(s.status),
  }));
  for (const r of rescueShows) {
    if (r.tvdb === null || seen.has(r.tvdb)) continue;
    seen.add(r.tvdb);
    shows.push({ tvdb: r.tvdb, imdb: null, title: r.title, status: 'not_started_yet',
      simklBucket: statusToSimkl('not_started_yet'), traktTreatment: statusToTrakt('not_started_yet') });
  }
...
export function buildPlanToWatch(refractSeries, rescueShows, movies) {
  ...
  const ptwMovies = movies.filter((m) => !m.watched)
    .map((m) => ({ tvdb: m.tvdb, imdb: m.imdb, uuid: m.uuid, title: m.title }));
  return { shows: shows.sort(byTvdb), movies: ptwMovies.sort(byTvdb) };
}

export function buildDropped(refractSeries) {
  return refractSeries.filter((s) => s.status === 'stopped')
    .map((s) => ({ tvdb: s.tvdb, title: s.title })).sort(byTvdb);
}
