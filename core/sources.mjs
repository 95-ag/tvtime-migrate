...
export function loadRefractSeries(text) {
  ...
  return parseCsvObjects(text).map((r) => ({
    tvdb: nullIfEmpty(r.tvdb_id),
    imdb: nullIfEmpty(r.imdb_id),
    title: r.title,
    status: r.status,
  }));
}
...
export function loadRefractMovies(text) {
  ...
  return parseCsvObjects(text).map((r) => ({
    uuid: nullIfEmpty(r.uuid),
    tvdb: nullIfEmpty(r.tvdb_id),
    imdb: nullIfEmpty(r.imdb_id),
    title: r.title,
    year: nullIfEmpty(r.year),
    watchedAt: normalizeWatchedAt(r.watched_at),
...
export function loadRescueEpisodes(text) {
  ...
  return parseCsvObjects(text).map((r) => ({
    showTvdb: nullIfEmpty(r.tvdb_show_id),
    title: r.show,
    season: Number(r.season),
    episode: Number(r.episode),
    epTvdb: nullIfEmpty(r.tvdb_episode_id),
    watchedAt: normalizeWatchedAt(r.watched_at),
